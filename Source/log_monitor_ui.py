...
from PyQt5.QtCore import Qt, QThread, pyqtSlot, QMutex, QMutexLocker, pyqtSignal
from typing import Callable
from combat_parser import Parser, CombatSession, Character, Ability, DamageComponent
import random
import os.path
...
class LogMonitorUI(QMainWindow):
    ...
    def __init__(self, parser: Parser):
        ...
        self.ability_tree_display.setHeaderLabels(["Name", "DPS", "Acc %", "Avg Per Hit", "Count", "Max", "Min"])
   ...
   # @pyqtSlot()
    ...
    def on_sig_periodic_update(self, data):
        print("Periodic Update Signal Received")
        self.combat_session_data = data
        self.repopulate_sessions(self.combat_session_data)
    ...
    # Create a function to set up test data
        def setup_test_data():
            ...
            for i in range(0, 4):
                ...
                for j in range(0, 3): #Combat Sessions
                    ...
                    test_sessions[i].add_character(Character(character))
                    for k in range(0, random.randint(3, 6)):
                        ability = Ability(pick_random_ability(), True)
                        test_sessions[i].chars[character].add_ability(ability, ability)
                        for l in range(0, random.randint(1, 2)):
                            damage = DamageComponent(pick_random_type(), random.randint(0, 100))
                            test_sessions[i].chars[character].abilities[ability].add_damage(damage, random.randint(0, 100))
        ...
        setup_test_data()
        with QMutexLocker(self.parser.mutex):
            self.parser.combat_session_data = test_sessions
